  ...
  config = mkIf cfg.enable (mkMerge [
    {
      ...
      # enableSshSupport sets SSH_AUTH_SOCK in .zshenv via `gpgconf`, which needs GNUPGHOME
      programs.zsh.envExtra = mkBefore ''
    ...
    (helpers.mkIfDarwin {
      launchd.agents.gpg-agent.config.ProgramArguments = mkForce [
